...
all_countries = movies_df['country'].apply(lambda x: ", ".join(x) if isinstance(x, list) else x).str.split(", ")
...
movies.loc[:,'duration_minutes'] = movies['duration'].str.extract(r'(\d+)', expand=False).astype(float)

movies_avg_duration_per_year = movies.groupby('release_year')['duration_minutes'].mean()
