...
min_row = temps_df.loc[temps_df['AvgTemperatureFahrenheit'] == min_temp]
max_row = temps_df.loc[temps_df['AvgTemperatureFahrenheit'] == max_temp]

min_temp_city = min_row['City'].values[0] 
min_temp_date = min_row['Date'].values[0]

max_temp_city = max_row['City'].values[0] 
max_temp_date = max_row['Date'].values[0]
...
for city in selected_cities:
    ...
    city_df_period = city_df[(city_df['Date'].dt.date >= start_date) & (city_df['Date'].dt.date <= end_date)]     # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
...
plt.title("Temperature Variation in Cities")   # TODO
...
for city in selected_cities:
    city_df = temps_df[temps_df['City'] == city].assign(Date=pd.to_datetime(temps_df['Date']))             # TODO: get a dataframe with the rows of the selected city
    city_df_period = city_df[(city_df['Date'].dt.date >= start_date) & (city_df['Date'].dt.date <= end_date)]    # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
    plt.hist(city_df_period['AvgTemperatureCelsius'], bins=20, edgecolor='black', alpha=0.7, label=city)                    # TODO: plot each city histogram in the same plot and use the label parameter to set the legend name for each city 

plt.title("Temperature Distribution in Cities")   # TODO
